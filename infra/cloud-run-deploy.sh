...
gcloud run deploy "$SERVICE" \
  ...
  --set-env-vars="^@^GITHUB_APP_ID=${GITHUB_APP_ID}@GITHUB_APP_PRIVATE_KEY_PATH=/secrets/app/app.pem@GITHUB_ISSUE_REPO=${GITHUB_ISSUE_REPO:-kimjy-st/QA_test}@GITHUB_TARGET_REPO=${GITHUB_TARGET_REPO}@GOOGLE_SERVICE_ACCOUNT_JSON_PATH=/secrets/sa/sa.json@GOOGLE_SPREADSHEET_ID=${GOOGLE_SPREADSHEET_ID}@MEMBERS_XLSX_PATH=/secrets/members/Members.xlsx@ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY}@GEMINI_API_KEY=${GEMINI_API_KEY:-}@SLACK_WEBHOOK_URL=${SLACK_WEBHOOK_URL}@CORS_ORIGINS=${CORS_ORIGINS:-http://localhost:3000}@APP_ENV=production@LOG_LEVEL=INFO"
